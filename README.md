...
| Type | Highlights |
| --- | --- |
| **Residential** | Real home ISP IPs, large rotating pool, geo targeting by country, region, city and ISP |
| **Premium Residential** | Selected residential pool for stricter traffic environments and higher-trust workflows |
| **ISP** | Static IPv4 addresses from real internet providers, long-session stability, dedicated to one customer |
...
- UDP over SOCKS5 on all products except US Residential
- **p0f network fingerprint switching** for Datacenter, ISP and Mobile proxies
- p0f support on all Datacenter and ISP locations
- Geo targeting by country, region, city, ISP and mobile carrier where available
- Dedicated IPs for Datacenter and ISP proxies, with no sharing between customers
- Sticky and rotating sessions
- API access for orders, balance checks and proxy workflow automation
...
**Not advertised for:** Residential · Premium Residential.
...
| Тип | Особенности |
| --- | --- |
| **Residential** | Реальные домашние IP от интернет-провайдеров, большой ротационный пул, геотаргетинг по стране, региону, городу и ISP |
| **Premium Residential** | Отобранный residential-пул для более строгих traffic-сценариев и задач, где важен высокий уровень доверия к IP |
| **ISP** | Статические IPv4-адреса от реальных интернет-провайдеров, стабильность для долгих сессий, выделение одному клиенту |
...
- Поддержка HTTP, SOCKS5 и **UDP**
- UDP over SOCKS5 на всех продуктах, кроме US Residential
- **p0f network fingerprint switching** для Datacenter, ISP и Mobile proxies
- p0f поддерживается на всех локациях Datacenter и ISP proxies
- Геотаргетинг по стране, региону, городу, ISP и мобильному оператору, где доступно
- Выделенные IP для Datacenter и ISP proxies, без шаринга с другими клиентами
- Sticky и rotating sessions
- API для заказов, проверки баланса и автоматизации proxy workflows
...
**Не заявлено для:** Residential · Premium Residential.
...
| Тип | Особливості |
| --- | --- |
| **Residential** | Реальні домашні IP від інтернет-провайдерів, великий ротаційний пул, геотаргетинг за країною, регіоном, містом та ISP |
| **Premium Residential** | Відібраний residential-пул для більш строгих traffic-сценаріїв і задач, де важливий високий рівень довіри до IP |
| **ISP** | Статичні IPv4-адреси від реальних інтернет-провайдерів, стабільність для тривалих сесій, виділення одному клієнту |
...
- Підтримка HTTP, SOCKS5 та **UDP**
- UDP over SOCKS5 на всіх продуктах, крім US Residential
- **p0f network fingerprint switching** для Datacenter, ISP та Mobile proxies
- p0f підтримується на всіх локаціях Datacenter та ISP proxies
- Геотаргетинг за країною, регіоном, містом, ISP та мобільним оператором, де доступно
- Виділені IP для Datacenter та ISP proxies, без спільного використання з іншими клієнтами
- Sticky та rotating sessions
- API для замовлень, перевірки балансу й автоматизації proxy workflows
...
| 类型 | 亮点 |
| --- | --- |
| **Residential** | 来自真实家庭 ISP 的 IP，大型轮换池，支持按国家、地区、城市和 ISP 进行地理定位 |
| **Premium Residential** | 精选住宅 IP 池，适用于更严格的流量环境和高信任度工作流 |
| **ISP** | 来自真实互联网服务商的静态 IPv4 地址，适合长会话，并且独享给单个客户使用 |
...
- 支持 HTTP、SOCKS5 和 **UDP**
- 除 US Residential 外，所有产品均支持 UDP over SOCKS5
- **p0f network fingerprint switching** 适用于 Datacenter、ISP 和 Mobile proxies
- Datacenter 与 ISP proxies 的所有地区均支持 p0f
- 可按国家、地区、城市、ISP 和可用的移动运营商进行地理定位
- Datacenter 与 ISP proxies 为独享 IP，不与其他客户共享
- 支持 sticky 与 rotating sessions
- 提供 API，用于订单、余额查询和 proxy workflow 自动化
...
**未声明支持：** Residential · Premium Residential.
