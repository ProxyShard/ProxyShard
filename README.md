...
| Type | Highlights |
| --- | --- |
| **[Residential](https://proxyshard.com/residential-proxy)** | Real home ISP IPs, large rotating pool, geo targeting by country, region and city |
| **[Premium Residential](https://proxyshard.com/residential-proxy)** | Selected residential pool with ISP-level targeting, for stricter traffic environments and higher-trust workflows |
| **[ISP](https://proxyshard.com/isp-proxy)** | Static IPv4 from genuine residential ISP allocations (not datacenter ranges relabeled as ISP), issued to one customer at a time — same dedicated model as Datacenter |
| **[Datacenter](https://proxyshard.com/datacenter-proxy)** | Fast dedicated IPv4 proxies, low latency, 10 Gbit/s channels, built for scale and automation |
| **[Mobile](https://proxyshard.com/mobile-proxy)** | 3G, 4G, 5G and LTE carrier IPs, country and operator targeting, sticky and rotating sessions |

[See full pricing →](https://proxyshard.com/#prices)
...
| --- | --- | --- |
| **Proxy Tester** | Batch checker for HTTP and SOCKS5 proxies: connectivity, latency, status code and UDP detection for SOCKS5. Up to 100 proxies per check, 1 request per minute. | [proxyshard.com/proxy-tester](https://proxyshard.com/proxy-tester) |
| **IP Checker** | Shows the visible IP, geolocation, WebRTC status, browser anonymity score and 20+ browser fingerprint signals. | [proxyshard.com/ip-checker](https://proxyshard.com/ip-checker) |
...
| Тип | Особенности |
| --- | --- |
| **[Residential](https://proxyshard.com/ru/residential-proxy)** | Реальные домашние IP от интернет-провайдеров, большой ротационный пул, геотаргетинг по стране, региону и городу |
| **[Premium Residential](https://proxyshard.com/ru/residential-proxy)** | Отобранный residential-пул с фильтрацией по ISP, для строгих traffic-сценариев и задач, где важен высокий уровень доверия к IP |
| **[ISP](https://proxyshard.com/ru/isp-proxy)** | Статические IPv4 из настоящих ISP-выделений (а не серверные подсети с переклеенным ярлыком ISP, как у многих на рынке), выдаются в одни руки — как и Datacenter |
| **[Datacenter](https://proxyshard.com/ru/datacenter-proxy)** | Быстрые выделенные IPv4-прокси, низкая задержка, 10 Гбит/с каналы, высокая скорость для автоматизации и больших объёмов |
| **[Mobile](https://proxyshard.com/ru/mobile-proxy)** | IP из 3G, 4G, 5G и LTE-сетей, таргетинг по стране и оператору, sticky и rotating sessions |

[Полный прайс →](https://proxyshard.com/ru/#prices)
...
**Доступно на всех продуктах, два исключения:**

- **Standart и Unmetered Residential:** UDP не работает только на USA-локациях — все остальные локации работают штатно
- **Premium Residential:** UDP не поддерживается ни на одной локации
...
| --- | --- | --- |
| **Proxy Tester** | Пакетная проверка HTTP и SOCKS5 proxies: подключение, latency, status code и UDP detection для SOCKS5. До 100 прокси за одну проверку, 1 запрос в минуту. | [proxyshard.com/proxy-tester](https://proxyshard.com/proxy-tester) |
| **IP Checker** | Показывает видимый IP, геолокацию, WebRTC status, browser anonymity score и 20+ browser fingerprint signals. | [proxyshard.com/ip-checker](https://proxyshard.com/ip-checker) |
...
| Тип | Особливості |
| --- | --- |
| **[Residential](https://proxyshard.com/ua/residential-proxy)** | Реальні домашні IP від інтернет-провайдерів, великий ротаційний пул, геотаргетинг за країною, регіоном і містом |
| **[Premium Residential](https://proxyshard.com/ua/residential-proxy)** | Відібраний residential-пул з фільтрацією за ISP, для строгих traffic-сценаріїв і задач, де важливий високий рівень довіри до IP |
| **[ISP](https://proxyshard.com/ua/isp-proxy)** | Статичні IPv4 зі справжніх ISP-виділень (а не серверні підмережі під виглядом ISP, як у багатьох на ринку), видаються в одні руки — як і Datacenter |
| **[Datacenter](https://proxyshard.com/ua/datacenter-proxy)** | Швидкі виділені IPv4-проксі, низька затримка, канали 10 Гбіт/с, висока швидкість для автоматизації та великих обсягів |
| **[Mobile](https://proxyshard.com/ua/mobile-proxy)** | IP із 3G, 4G, 5G та LTE-мереж, таргетинг за країною й оператором, sticky та rotating sessions |

[Повний прайс →](https://proxyshard.com/ua/#prices)
...
> **Наразі ProxyShard — єдиний провайдер, у якого підміна p0f працює на ISP і Datacenter проксі, причому реалізована по-справжньому: підмінюється реальний TCP/IP-відбиток, а не просто ярлик у дашборді.**
...
**Доступно на всіх продуктах, два винятки:**

- **Standart і Unmetered Residential:** UDP не працює лише на USA-локаціях — на всіх інших локаціях працює штатно
- **Premium Residential:** UDP не підтримується на жодній локації
...
| --- | --- | --- |
| **Proxy Tester** | Пакетна перевірка HTTP та SOCKS5 proxies: підключення, latency, status code та UDP detection для SOCKS5. До 100 проксі за одну перевірку, 1 запит на хвилину. | [proxyshard.com/proxy-tester](https://proxyshard.com/proxy-tester) |
| **IP Checker** | Показує видимий IP, геолокацію, WebRTC status, browser anonymity score та 20+ browser fingerprint signals. | [proxyshard.com/ip-checker](https://proxyshard.com/ip-checker) |
...
| 类型 | 亮点 |
| --- | --- |
| **[Residential](https://proxyshard.com/cn/residential-proxy)** | 来自真实家庭 ISP 的 IP，大型轮换池，支持按国家、地区和城市进行地理定位 |
| **[Premium Residential](https://proxyshard.com/cn/residential-proxy)** | 精选住宅 IP 池，支持按 ISP 定向，适用于更严格的流量环境和高信任度工作流 |
| **[ISP](https://proxyshard.com/cn/isp-proxy)** | 来自真实 ISP 分配的静态 IPv4 地址(并非伪装成 ISP 的数据中心 IP 段),与 Datacenter 一样,单客户独享 |
| **[Datacenter](https://proxyshard.com/cn/datacenter-proxy)** | 高速独享 IPv4 代理，低延迟，10 Gbit/s 通道，适合规模化任务和自动化 |
| **[Mobile](https://proxyshard.com/cn/mobile-proxy)** | 来自 3G、4G、5G 和 LTE 运营商网络的 IP，支持国家和运营商定位、sticky 与 rotating sessions |

[查看完整价格 →](https://proxyshard.com/cn/#prices)
...
> **目前 ProxyShard 是唯一在 ISP 与 Datacenter 代理上提供 p0f 网络指纹切换功能的供应商,而且我们的实现会真正改写 TCP/IP 指纹,而非仅在仪表盘上贴个标签。**
...
- **Standart 与 Unmetered Residential:** 仅 USA 地区不支持 UDP — 其他所有地区均可正常使用
- **Premium Residential:** 任何地区均不支持 UDP
...
| 工具 | 功能 | 打开 |
| --- | --- | --- |
| **Proxy Tester** | HTTP 和 SOCKS5 proxies 批量检测：连接状态、latency、status code 以及 SOCKS5 的 UDP detection。每次最多 100 个代理，每分钟 1 次请求。 | [proxyshard.com/proxy-tester](https://proxyshard.com/proxy-tester) |
| **IP Checker** | 显示可见 IP、地理位置、WebRTC status、browser anonymity score 和 20+ browser fingerprint signals。 | [proxyshard.com/ip-checker](https://proxyshard.com/ip-checker) |
